...
int32_t I2C_Data_Send(uint8_t byte, uint8_t* ack){
  TWDR = byte; //data
  TWCR = 0x84; //rst int., enable
  while((TWCR & 0x80) == 0x00); //wait till data sent
  if((TWSR & 0xF8) == 0x28){
    *ack = I2C_ACK; //data sent, ACKed
    return I2C_SUCCESS; 
  }
  else if((TWSR & 0xF8) == 0x30){
    *ack = I2C_NACK; //data sent, ACKed
    return I2C_SUCCESS; 
  } //data sent, not ACKed
  else return I2C_ERROR_DATA_WRITE;
}
int32_t I2C_Data_Read(uint8_t* data){
  ...
  *data = TWDR; //data
  ...
  else return I2C_ERROR_DATA_READ;
}
